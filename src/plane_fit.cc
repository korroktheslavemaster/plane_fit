...
void DepthToPointCloud(cv::Mat depth_image, vector<SpaceCoord>& point_cloud,
...
SpaceCoord PlaneFitDepthImage(cv::Mat depth_image,
  ...
  DepthToPointCloud(depth_image, point_cloud, intrinsics, step);

  return RansacPlaneFit(point_cloud, iterations, inlier_distance,
    min_distance, max_distance, centroid, plane_points);
}
...
SpaceCoord RansacPlaneFit(vector<SpaceCoord> point_cloud,
  ...
  // copy over the points to plane_points
  if (plane_points) {
    *(plane_points) = best_points;
  }
